...
DEFAULT_OUTPUT = Path(os.environ.get("FRUITS_OUTPUT", CONFIG.get("default_output", r"X:\brain\00_WORKFLOWS\paper-proof-grader\OUTPUT\fruits_of_spirit")))
...
class LocalScoreModule:
    ...
    @classmethod
    def _term_hits(cls, tokens: list[str], terms: set[str]) -> int:
    ...
    def _score_text(cls, text: str) -> dict:
        ...
        fruit = cls._clamp(cls._term_hits(tokens, FRUIT_TERMS) / max(3.0, token_count / 35.0))
        anti = cls._clamp(cls._term_hits(tokens, ANTI_FRUIT_TERMS) / max(3.0, token_count / 35.0))
        ground = cls._clamp(cls._term_hits(tokens, GROUNDING_TERMS) / max(3.0, token_count / 40.0))
        contra = cls._clamp(cls._term_hits(tokens, CONTRADICTION_TERMS) / max(2.0, token_count / 50.0))
        if len(sentences) >= 2:
