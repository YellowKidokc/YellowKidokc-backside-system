...
for dir in models captures understood embeddings ratings knowledge memory digests logs github deploy obsidian; do
  mkdir -p "$BRAIN/$dir"
done
echo "  Done."
...
if curl -s --connect-timeout 3 http://localhost:11434/api/tags > /dev/null 2>&1; then
  ...
  OLLAMA_CONTAINER="$(docker ps --format '{{.Names}}' | grep -i ollama | head -n 1 || true)"
  if [ -z "$OLLAMA_CONTAINER" ]; then
    echo "  WARNING: could not find a running Ollama container name"
  fi
  # Check for moondream
  if curl -s http://localhost:11434/api/tags | grep -q "moondream"; then
    ...
    if [ -n "$OLLAMA_CONTAINER" ]; then
      docker exec "$OLLAMA_CONTAINER" ollama pull moondream
    else
      echo "  Skipped moondream pull: no container name found"
    fi
  fi
  # Check for llava
  if curl -s http://localhost:11434/api/tags | grep -q "llava"; then
    ...
    if [ -n "$OLLAMA_CONTAINER" ]; then
      ...
      echo "  Skipped llava pull: no container name found"
    fi
  fi
else
...
if docker ps --format '{{.Names}}' | grep -q pil-infinity; then
  ...
  docker run -d --name pil-infinity \
    ...
    v2 --model-id sentence-transformers/all-MiniLM-L6-v2 --port 7997 --engine torch
...
if docker ps --format '{{.Names}}' | grep -q pil-api; then
  ...
  docker run -d --name pil-api \
    ...
    -e OBSIDIAN_SOURCE_DIR=/data/obsidian \
